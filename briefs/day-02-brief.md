...
[← Previous: Day 1 - Introduction](./day-01-brief.md) | [Course Home](../README.md) | [Next: Day 3 - Geometries →](./day-03-brief.md)
...
**Material**: The appearance - how the surface reflects light, its color, texture, and visual properties. Materials range from simple (`MeshBasicMaterial` - no lighting) to complex (`MeshStandardMaterial` - realistic physically-based rendering).
...
This loop creates smooth animation. Even if you want a static scene (no animation), you still need this loop to handle interactions like resizing the window.
...
| **BoxGeometry** | Built-in geometry for creating rectangular boxes/cubes | `new THREE.BoxGeometry(1, 1, 1)` creates a 1x1x1 cube |
...
| **requestAnimationFrame** | Browser API for scheduling smooth animations | Syncs with screen refresh rate |
...
// 2. Create the camera
// Parameters: FOV (field of view in degrees), aspect ratio, near clipping plane, far clipping plane
...
**What's happening:** This is the "Hello World" of Three.js. We create a scene, camera, and renderer (the big three), then add a green cube to the scene. The animation loop rotates the cube slightly each frame, creating smooth rotation. The cube rotates around both X and Y axes simultaneously, giving it a tumbling effect.
...
**What's happening:** When the window resizes, we need to update two things: the camera's aspect ratio (so the view doesn't stretch) and the renderer's size (so it matches the new window dimensions). The `updateProjectionMatrix()` call is crucial - it recalculates the camera's internal projection matrix based on the new aspect ratio.
...
**What's happening:** Materials have many properties beyond color. `transparent` and `opacity` let you create see-through objects. `wireframe` shows just the edges of the geometry, which is great for understanding the mesh structure. You can use hex colors (`0xRRGGBB`) or CSS color names.
...
**Expected Output:**
A green cube rotating smoothly in the center of the screen against a black background.
...
Rotation values are in radians per frame. Smaller = slower, bigger = faster.
</details>
...
**Expected Output:**
Four cubes in a horizontal and vertical arrangement, three rotating at different speeds, one static wireframe.
...
1. **Creating a Scene** — Three.js Manual
   ...
   - Why: Official guide with interactive examples

2. **Understanding the Camera** — Three.js Docs
   - 🔗 https://threejs.org/docs/#api/en/cameras/PerspectiveCamera
   - Why: Deep dive into camera parameters (FOV, near, far)

### Video Resources

3. **Learn Three.js from Scratch** — Chris Courses
   - 🎥 https://www.youtube.com/watch?v=YK1Sw_hnm58 (⚠️ VERIFIED, 0:00-15:00)
...
4. **requestAnimationFrame Explained** — MDN
   ...
   - Why: Understand the animation timing mechanism
...
**Tomorrow:** [Day 3: Exploring Geometries](./day-03-brief.md) — Learn about all the built-in shapes Three.js provides and how to customize them

Now that you can create and render objects, tomorrow we'll explore the variety of geometries Three.js offers - spheres, cylinders, toruses, and more. You'll learn how to control their parameters and combine multiple shapes.
